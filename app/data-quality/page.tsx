    ...
    <AppShell area="data-quality">
      <Section title="데이터 점검" description="운영 데이터의 필수값, 중복, 참조 불일치, 예약 충돌, 서버 감사 로그를 확인합니다.">
        <div className="grid gap-4 md:grid-cols-6">
          <SummaryCard label="데이터 소스" value={sourceLabel[state.source]} helper={state.source === "server" ? "별도 서버 세션 사용" : state.hasLiveSession ? "전환 세션 사용" : "실사용 로그인 필요"} />
        ...
        <div className="rounded-2xl border border-line bg-white p-5 shadow-card">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            ...
            <Badge tone={auditState.source === "server" ? "good" : auditState.source === "fallback" ? "danger" : "warn"}>
              {auditState.source === "server" ? `${auditState.logs.length}건` : "서버 전용"}
            </Badge>
          </div>
...
function buildOperationsReadinessItems(
  ...
  return [
    ...
    },
    {
      ...
      detail: accounts ? "Admin, Manager, Coach, Artist 계정 현황이 서버 요약에 포함되어 있습니다." : "서버 연결 후 실제 운영 계정을 등록해야 합니다.",
