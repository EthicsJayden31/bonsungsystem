...
export default function AttendancePage() {
  ...
  const canWriteAttendance = hasStagePermission(user ?? role, "writeLessonLogs") || hasStagePermission(user ?? role, "manageOperations");

  return (
    <AppShell area="attendance">
      <ResourcePage
        title="출결 관리"
        description={source === "server" ? "본성 스테이지 서버의 출결 데이터를 표시합니다." : source === "live" ? "전환 세션의 수업/레슨노트 데이터에서 출결 상태를 계산해 표시합니다." : source === "fallback" ? "출결 데이터를 불러오지 못했습니다. 서버 연결과 권한을 확인해야 합니다." : "출결 데이터를 확인하고 있습니다."}
