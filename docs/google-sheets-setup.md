...
본성 스테이지는 Vercel 또는 별도 본성 스테이지 API 서버에서 운영 화면을 실행하고, 운영 데이터는 Google Sheets 계열 저장소에 보관할 수 있습니다. 브라우저가 직접 Google Sheet 주소를 입력하거나 더미 프리뷰 화면을 여는 구조가 아닙니다.

## 권장 방식 1: 서버 측 Google Sheets 어댑터

서버가 Google Sheets API를 사용해 `_stage_state`와 `_stage_sessions` 시트에 전체 상태와 세션을 저장합니다.
...
사용자 입력은 먼저 본성 스테이지 API가 PostgreSQL에 저장하고, 별도 동기화 작업이 Apps Script Web App으로 미러링합니다.

필수 환경 변수 예시:

```text
BONSUNG_STORAGE_DRIVER=postgres
BONSUNG_DATABASE_URL=<postgres-url>
...
Apps Script 코드는 `apps-script/stage-apps-script.gs`를 사용합니다. 이전 `google-apps-script/Code.gs` 경로는 제거되었습니다.
...
기존 `owner`, `teacher`, `student` 값이 들어온 경우 Apps Script와 본성 스테이지 서버는 각각 `admin`, `coach`, `artist`로 정규화합니다.
...
- 운영 점검은 공식 UI와 본성 스테이지 API를 통해 진행합니다.
