# 본성 스테이지 외부 서버 후보 검토 기준

본성 스테이지의 공개 경로는 Apps Script가 아니라 별도 서버를 기준으로 운영합니다. Base44, Lovable, 별도 Node 서버 중 어떤 후보를 사용하더라도 아래 계약을 통과해야 실제 학원 운영 서버로 볼 수 있습니다.

## 공통 통과 기준

- `https` 공개 URL을 제공해야 합니다.
- `BONSUNG_API_BASE_URL` 또는 `NEXT_PUBLIC_BONSUNG_API_BASE_URL`로 Next UI에 연결할 수 있어야 합니다.
- `pnpm run verify:stage-server`를 외부 URL 대상으로 통과해야 합니다.
- `pnpm run verify:stage-release`를 운영 배포 설정으로 통과해야 합니다.
...
- 백엔드 함수는 본성 스테이지 API 계약과 같은 이름 또는 같은 동작을 제공해야 합니다.
...
Lovable은 빠르게 풀스택 후보를 만들고 본성 스테이지 계약과 비교하는 용도로 검토합니다.

- UI를 새로 만드는 목적보다 본성 스테이지 서버 API 후보를 빠르게 세우는 목적이 우선입니다.
- 생성된 앱은 공식 Next UI를 대체하지 않고, `/auth/login`, `/bootstrap`, `/actions/{action}` 같은 서버 계약을 맞추는지 확인합니다.
- 후보가 만들어지면 외부 URL을 `BONSUNG_SERVER_VERIFY_BASE_URL`에 넣고 `pnpm run verify:stage-server`를 실행합니다.
- 검증 통과 전에는 GitHub Pages 운영 변수 `BONSUNG_API_BASE_URL`에 연결하지 않습니다.

## 별도 Node 서버 후보 사용 기준

현재 `server/stage-server.mjs`는 운영 서버 확정 전 계약 검증용 기준 서버입니다.
...
2. 후보 서버에 본성 스테이지 필수 API를 먼저 만듭니다.
3. 외부 URL로 `pnpm run verify:stage-server`를 통과시킵니다.
4. GitHub 저장소 변수 `BONSUNG_API_BASE_URL`에 검증된 HTTPS URL을 등록합니다.
5. `pnpm run verify:stage-release`와 Pages 배포 검증을 통과한 뒤 운영 연결로 전환합니다.

## Docker/Render 별도 서버 후보

외부 서비스 선택 전에도 본성 스테이지 서버 계약을 실제 호스팅 후보에서 시험할 수 있도록 `Dockerfile`, `.dockerignore`, `render.yaml`을 추가했습니다.

- Docker 이미지는 `server/stage-server.mjs`만 실행합니다.
