## 2026-07-02 15:32 역할별 홈 화면과 사이드바 권한 개선

Version.3 공식 Next UI는 계정 역할에 따라 홈 화면 제목, 설명, 빠른 이동, 통계 카드, 상세 패널을 다르게 보여줍니다. 대표는 전체 운영과 데이터 점검, 매니저는 학생·상담 운영, 강사는 담당 수업과 출결·레슨노트, 수강생은 본인 수업·공지·상담요청·레슨노트·연습실 예약을 중심으로 봅니다.

PC 사이드바는 권한별로 허용된 메뉴만 표시하고, 상위 그룹의 접기/펼치기 상태를 브라우저에 저장합니다. 사이드바 검색은 현재 화면에 실제로 보이는 그룹명과 허용된 하위 메뉴만 대상으로 하므로, 수강생이 `수납`이나 `수강`처럼 권한 밖 운영 메뉴를 검색해도 해당 링크가 노출되지 않습니다.

수강생 계정은 기본 권한에서 `viewPayments=false`이며 `/bootstrap` 응답에서도 `payments`와 `enrollments`를 빈 배열로 받습니다. 즉 테스트모드와 실제 서버 모드 모두에서 수강생에게 등록·수납 운영 데이터가 내려가지 않습니다. 대표 계정은 같은 화면에서 `/enrollments/`, `/payments/`, `/accounts/` 링크가 계속 보이므로 역할별 차이를 브라우저에서 확인할 수 있습니다.

검증은 `pnpm run typecheck`, `pnpm run lint`, `pnpm run verify:surfaces`, `pnpm run verify:version3-server`, `pnpm run verify:version3-test-mode`, `pnpm run build:pages`로 통과했습니다. 브라우저 점검에서는 `/version3-test/` 학생 로그인 후 홈/메뉴/검색에서 수납·수강 링크와 회계 패널이 없고, 대표 로그인 후 수강·수납·계정 링크가 보이는 것을 확인했습니다.
